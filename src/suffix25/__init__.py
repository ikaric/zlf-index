...
try:
    ...
    # fmt: off
    from suffix25._core import (  # type: ignore[import-untyped, unused-ignore, import-not-found]
        BM25Index,
        ...
        suffix25 as _suffix25,
    )
    # fmt: on
except ImportError:
    # Fall back to pure-Python implementation
    # fmt: off
    from suffix25.core import State, BM25Index  # type: ignore[assignment, unused-ignore]
    from suffix25.core import (
        build_suffix_automaton as _build_suffix_automaton,
        suffix25 as _suffix25
    )
    # fmt: on
    ...
    def _batch_scores(query: str, automatons: list[State]) -> list[float]:
        return [_suffix25(query, a) for a in automatons]
...
class Document:
    ...
    def score(self, query: str, alpha: float = 0.5) -> float:
        ...
        Alpha is ignored.
        """
        return float(_suffix25(query, self._state))
...
def score(  # noqa: C901
    ...
    if isinstance(target, str):
        return float(_suffix25(query, target))
